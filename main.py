import os,json,urllib,tls_client,base64,requests
...
if config['proxy'] != "":
	...
	proxies_two = f"http://{config['proxy']}"
else:
	proxies = None
	proxies_two = ""
...
while True:
    try:
        response = requests.get("https://raw.githubusercontent.com/EffeDiscord/discord-api/main/fetch", proxies=proxies).json()
...
def captcha_solver(url, key, captcha_rqdata):
...
while True:
    try:
        server_info = requests.get(f'https://discord.com/api/v9/invites/{invite_code}', proxies=proxies)
...
while join < total_token:
	try:
		...
		client.get(f'https://discord.com/api/v9/invites/{invite_code}', headers=headers, params={'inputValue': invite_code, 'with_counts': 'true', 'with_expiration': 'true'}, proxy=proxies_two)
		headers["x-context-properties"] = encoded_xcontent
		response = client.post(f"https://discord.com/api/v9/invites/{invite_code}", json={}, headers=headers, proxy=proxies_two)
		if response.status_code == 400:
			...
			client.get(f'https://discord.com/api/v9/invites/{invite_code}', headers=headers, params={'inputValue': invite_code, 'with_counts': 'true', 'with_expiration': 'true'}, proxy=proxies_two)
			response_captcha = client.post(f"https://discord.com/api/v9/invites/{invite_code}", json={"captcha_key": captcha_solver("https://discord.com", f"{response.json()['captcha_sitekey']}", response.json()['captcha_rqdata']), 'captcha_rqtoken': response.json()['captcha_rqtoken']}, headers=headers, proxy=proxies_two)
			if response_captcha.status_code == 200:
				...
				if 'show_verification_form' in body:
					get_rules = client.get(f"https://discord.com/api/v9/guilds/{guild_id}/member-verification?with_guild=false", headers=headers, proxy=proxies_two).json()
					response2 = client.put(f"https://discord.com/api/v9/guilds/{guild_id}/requests/@me", headers=headers, json=get_rules, proxy=proxies_two)
					if response2.status_code == 201 or response2.status_code == 204:
						...
						print(f"{Fore.LIGHTRED_EX} [!] {token[:50]}****** not accepted the rules! {Fore.RESET}({Fore.LIGHTBLACK_EX}{response2.text}{Fore.RESET})")
				if emoji_bypass == "yes":
					response3 = client.put(f"https://discord.com/api/v9/channels/{channel_id}/messages/{message_id}/reactions/{emoji_converted}/%40me", headers=headers, proxy=proxies_two)
					if response3.status_code == 201 or response3.status_code == 204:
						...
						print(f"{Fore.LIGHTRED_EX} [!] {token[:50]}****** can't reacted to the emoji! {Fore.RESET}({Fore.LIGHTBLACK_EX}{response3.text}{Fore.RESET})")
			...
			print(f"{Fore.LIGHTRED_EX} [!] {token[:50]}****** not joined! {Fore.RESET}({Fore.LIGHTBLACK_EX}{response.text}{Fore.RESET})")
